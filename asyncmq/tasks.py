...
def task(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        Args:
            func: The original asynchronous or synchronous function being decorated
                  as an asyncmq task.
        ...
        Returns:
            ...
            reporting.
        """
        # Generate a unique ID for the task based on its module and name.
            ...
            **kwargs: Any
            ...
            Args:
                backend: An object providing the necessary interface for
                         interacting with the queue, including `add_dependencies`,
                         `enqueue_delayed`, and `enqueue`. Typed as `Any`.
            ...
            """
            # Create a Job instance with the task ID, arguments, and configuration.
            job = Job(
                task_id=task_id,
            ...
            # If the job has dependencies, register them with the backend.
            if job.depends_on:
            ...
            if delay and delay > 0:
                ...
                # Enqueue the job to be processed after the specified delay.
                await backend.enqueue_delayed(queue, job.to_dict(), run_at)
            else:
                # Enqueue the job for immediate processing.
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            async with anyio.create_task_group() as tg:
                if progress:
                    # Define the report_progress callback that emits an event.
                    def report(pct: float, data: Any = None) -> None:
                    ...
                    # Call the original function, injecting the report_progress callback.
                    result = func(*args, report_progress=report, **kwargs)
                else:
                    # Call the original function without the progress callback.
                    result = func(*args, **kwargs)

                # Check if the result is an awaitable (i.e., the original function was async).
                if inspect.isawaitable(result):
                    # If it's awaitable, await the result directly.
                    return await result
                else:
                    # If it's not awaitable (i.e., the original function was synchronous),
                    # run it in a thread to prevent blocking the event loop.
        ...
        TASK_REGISTRY[task_id] = {
            "func": wrapper,          # Store the wrapped function
            "queue": queue,           # Store the target queue name
            "retries": retries,       # Store the configured retry count
            "ttl": ttl,               # Store the configured TTL
            "progress_enabled": progress # Store whether progress reporting is enabled
        }
