...
| Scheduling | scheduler ownership | Supported | Backend-managed repeatables are advanced under a queue-scoped scheduler lock. |
...
- AsyncMQ exposes a derived `waiting-children` inspection bucket from dependency metadata, but it does not persist BullMQ's separate Redis-native `paused` or `prioritized` job buckets. Queue pause remains queue-level control, and priority remains ordering metadata on waiting jobs.
...
- AsyncMQ stores deduplication metadata on job payloads instead of Redis-only side keys, which keeps the feature portable across built-in backends.
